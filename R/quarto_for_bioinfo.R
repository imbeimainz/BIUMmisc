create_bioinfo_html <- function(file_name = NULL,
  ...
  # create folder
  ## TODO: possibly cover the fact it is going to be a vignette?
  if(!file.exists(paste0("_extensions/", ext_name))) dir.create(paste0("_extensions/", ext_name))

  # copy from internals
  file.copy(
    from = system.file(paste0("extdata/_extensions/", ext_name), package = "BIUMmisc"),
  ...
  file.copy("_extensions/bioinfo-html/bioinfo-html-report-template.qmd",
            paste0(file_name, ".qmd", collapse = ""))

  # open the new file in the editor
  file.edit(paste0(file_name, ".qmd", collapse = ""))

  # TODO: some better messages all around, maybe?

  cli::cli_alert_success("Have fun with your report!")
}
